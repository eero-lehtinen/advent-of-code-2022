...
fn main() {
	let mut numbers: Vec<(usize, i64)> = include_str!("../input.txt")
		.lines()
		.map(|s| s.parse::<i64>().unwrap() * 811_589_153)
		.enumerate()
		.collect();
	...
	for _ in 0..10 {
		for original_index in 0..numbers.len() {
			...
			let new_i = index as i64 + value;
			let new_i = new_i.rem_euclid(numbers.len() as i64 - 1);
